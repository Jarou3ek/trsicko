...
for source in sorted((SOURCE / 'Portréty').iterdir(), key=lambda p: int(p.name.split('.')[0]) if not p.name.startswith('.') else 999):
    ...
    candidate_page = f'''<main id="obsah" class="article"><a class="back" href="index.html#kandidati">← Zpět na kandidátku</a><p class="eyebrow">Kandidátka 2026 · č. {number}</p><h1>{escape(name)}</h1><p class="source-note">Kandidátský profil sdružení Tršicko – obce pro život.</p><img src="assets/{asset}" alt="Kandidátský profil: {escape(name)} – zkušenosti a představení" width="1100" height="1650"><p><a href="assets/{asset}">Otevřít profil v plné velikosti ↗</a></p></main>'''
...
<section class="section" id="kandidati"><div class="wrap"><div class="section-head"><div><p class="eyebrow">Kandidátka 2026</p><h2>Kandidáti sdružení</h2></div><p>Jedenáct kandidátů do zastupitelstva obce.</p></div><div class="candidates">''' + ''.join(cards) + '''</div></div></section>
